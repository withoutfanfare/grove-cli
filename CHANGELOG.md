...
- **zsh `$path` clobbering in `check_disk_space` and `lookup_worktree_path`** - A `local path=…` declaration shadowed zsh's special `path` array (tied to `$PATH`), emptying `PATH` inside the function. This broke `grove add` with `command not found: tail`/`awk` in shells where those tools were not already command-hashed (fresh non-interactive shells, cron, CI). Renamed the locals to `target_path`/`wt_path`. The bash test-mirrors could not catch this because `path` is not special in bash
