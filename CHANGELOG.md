...
- **Lifecycle JSON remains parseable** - New-branch `add --json` notices no longer contaminate stdout, and dirty `rm --json` requests return `DIRTY_WORKTREE` without prompting. Successful result and error shapes remain unchanged
...
### Changed
- **`grove rm` runs the worktree removal gate** - before the pre-rm hooks and before git touches anything, `rm` asks `wt-removal-check` whether removing the worktree would lose anything: uncommitted changes, commits no remote has, or a live agent session working there. A block prints exactly what would be lost. `-f` does not bypass it (forcing git and accepting the loss of unsaved work are different decisions): interactively the loss is shown and you are asked `Remove anyway? [y/N]`; with `--json` the removal fails with `REMOVAL_BLOCKED` (exit 6) and the loss in the message, and there is no override. A missing `wt-removal-check` also refuses, because "no gate ran" and "the gate passed" must never look alike; point `GROVE_REMOVAL_CHECK_BIN` at it if it lives somewhere unusual. This replaces both the `-f`-skippable uncommitted-changes prompt and the Worktree Ledger gate (see Removed)
...
### Removed
- **The Worktree Ledger integration** - `grove rm` no longer consults Waypoint's `way` CLI, `add` no longer registers worktrees and `move` no longer reconciles them, and the `ledger` object is gone from `ls --json` and `status --json`. With it go `--ledger-ack`, `LEDGER_INTEGRATION`, `GROVE_WAY_BIN`, the `GROVE_LEDGER_ACK` hook variable and the `LEDGER_BLOCKED` and `DIRTY_WORKTREE` error codes, all replaced by the removal gate (see Changed). `GROVE_FORCE` is still exported to hooks
