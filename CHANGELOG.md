...
- **Optional `ledger` object in `grove status --json`** - nested and additive, so existing consumers are unaffected. `available: false` carries an `unavailable_reason` and means *unknown*, never "nothing at risk"
